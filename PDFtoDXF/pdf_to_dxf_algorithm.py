...
class PdfToDxfAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ezdxf_req = dependencies.get_ezdxf_requirement()
        # Escape for HTML display in QGIS help
        ezdxf_req_display = ezdxf_req.replace('<', '&lt;').replace('>', '&gt;')
        return self.tr(f"Converts a PDF file to a DXF file using PyMuPDF and ezdxf.\n"
                       f"Required dependencies: pymupdf, {ezdxf_req_display}\n\n"
                       ...
                       f'pip install pymupdf "{ezdxf_req_display}"')
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            import fitz
            if not hasattr(fitz, 'open'):
                 raise ImportError("Incorrect 'fitz' package installed. Please install 'pymupdf'.")
            try:
                import ezdxf
            except (ImportError, AttributeError, Exception):
                 raise ImportError("Incompatible 'ezdxf' version. Please reinstall dependencies.")
            import ezdxf
